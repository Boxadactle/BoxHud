...
public class CenterModifier implements PositionModifier {
    @Override
    public Vec2<Integer> translateVector(Vec2<Integer> original, Dimension<Integer> window) {
        ...
        return new Vec2<>(windowWidth / 2 + x, windowHeight / 2 + y);
    }
    ...
    public Vec2<Integer> getStartCorner(Rect<Integer> rect) {
        return new Vec2<>(rect.getX() + rect.getWidth() / 2, rect.getCenterY());
    }
}
